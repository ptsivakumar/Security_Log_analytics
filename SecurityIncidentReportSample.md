...
On August 10, 2025, a security incident was identified where a user, identified as "User A," was able to retrieve sensitive data belonging to another user, "User B." The incident occurred due to an **authorization logic flaw** in a microservice. The **Web Application Firewall (WAF)** and **API Gateway** successfully authenticated the user but failed to prevent the unauthorized data query. The incident has been contained with a hotfix, and the vulnerability has been patched. Affected data includes personal and financial information.
...
| :--- | :--- | :--- | :--- |
| **10:00:15 AM** | **API Gateway** | `INFO: Request ID: 67890. User ID: User A. IP: 203.0.113.10. Endpoint: /api/accounts/UserB/details.` | User A authenticates successfully and sends a request for User B's data. The API Gateway forwards the request. |
